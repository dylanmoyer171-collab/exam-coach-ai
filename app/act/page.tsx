...
import { ACT_CONFIG } from "@/lib/exam-data";
import { generateStudyPlan, type StudyPlan } from "@/lib/study-plan-generator";
import { predictACTScore, type ScorePrediction } from "@/lib/score-predictor";
import { ArrowRight, AlertCircle } from "lucide-react";

export default function ACTPage() {
  const [currentScore, setCurrentScore] = useState(25);
  const [targetScore, setTargetScore] = useState(32);
  const [testDate, setTestDate] = useState("");
  const [hoursPerWeek, setHoursPerWeek] = useState(8);
  const [weakAreas, setWeakAreas] = useState<string[]>(["Science Reasoning", "Math Algebra"]);
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!currentScore || currentScore < 1 || currentScore > 36) {
      newErrors.push("Current score must be between 1–36");
    }
    if (!targetScore || targetScore < 1 || targetScore > 36) {
      newErrors.push("Target score must be between 1–36");
    }
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-4 py-8 sm:px-8">
      <div className="mx-auto max-w-6xl">
        {/* Header */}
        <div className="mb-8">
          <Badge variant="secondary" className="mb-4">ACT</Badge>
          <h1 className="text-4xl font-bold text-white mb-2">ACT Study Plan Generator</h1>
          <p className="text-slate-400">
            English, Math, Reading, Science, and optional Writing (1–36)
          </p>
        </div>
            ...
            <Card className="sticky top-24">
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  {/* Current Score */}
                  <div>
                    <Label htmlFor="current">Current Composite Score</Label>
                    <Input
                      id="current"
                      type="number"
                      min="1"
                      ...
                      className="mt-1"
                    />
                    <p className="text-xs text-slate-400 mt-1">1–36</p>
                  </div>
            ...
            {submitted && studyPlan && prediction ? (
              ...
              <Card className="h-96 flex items-center justify-center">
                <CardContent className="text-center">
                  <p className="text-slate-400">Fill in your information to generate a personalized study plan.</p>
                </CardContent>
              </Card>
            ) : null}
          </div>
